...
class PathTracingPass : public ::RenderPass, inherit_shared_from_this<::RenderPass, PathTracingPass>
{
public:
    using SharedPtr = std::shared_ptr<PathTracingPass>;
    using SharedConstPtr = std::shared_ptr<const PathTracingPass>;

    static SharedPtr create(const std::string &outChannel) { return SharedPtr(new PathTracingPass(outChannel)); }
    virtual ~PathTracingPass() = default;

protected:
	PathTracingPass(const std::string &outChannel) : mOutputTextureName(outChannel),
		::RenderPass("Path Tracing", "Path Tracing Options") {}
