...
void shootIndirectRay(inout IndirectRayPayload payload)
{
...
void updateRayData(inout IndirectRayPayload rayData, float3 color, float3 hit, float3 L, float3 N, float3 V, float3 dif, float3 spec, float rough, float pdfF, float pdfB)
{
    // update brdf
    rayData.color *= color;
...
void handleIndirectRayHit(float3 hit, float3 N, float3 noNormalN, float3 V, float3 dif, float3 spec, float rough, inout IndirectRayPayload rayData)
{
    ...
    if (chooseDiffuse)
    {
        ...
        updateRayData(rayData, color, hit, L, N, V, dif, spec, rough, pdfForward, pdfBackward);
    }
